...
class HRcloudNet(nn.Module):
    ...
    def forward(self, input):
        ...
        out = F.interpolate(x, size=input.size()[2:],
...
class DoubleConv(nn.Sequential):
    def __init__(self, in_channels, out_channels, mid_channels=None):
        ...
        super(DoubleConv, self).__init__(
            nn.Conv2d(in_channels+out_channels, mid_channels,
...
class Corr(nn.Module):
    def __init__(self, nclass=2):
        ...
        self.conv1 = nn.Conv2d(
            48, self.nclass, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def forward(self, feature_in, out):
        ...
        # out = [4 2 352 352]
        h_in, w_in = math.ceil(
            feature_in.shape[2] / (1)), math.ceil(feature_in.shape[3] / (1))
        out = F.interpolate(out.detach(), (h_in, w_in),
                            mode='bilinear', align_corners=True)
        feature = F.interpolate(feature_in, (h_in, w_in),
                                mode='bilinear', align_corners=True)
        ...
        corr_map = torch.matmul(f1.transpose(1, 2), f2) / \
            torch.sqrt(torch.tensor(f1.shape[1]).float())
        corr_map = F.softmax(corr_map, dim=-1)
        # out_temp 2 2 484
        # corr_map 4 484 484
        out = rearrange(torch.matmul(out_temp, corr_map),
                        'n c (h w) -> n c h w', h=h_in, w=w_in)
