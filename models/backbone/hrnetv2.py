...
class StageModule(nn.Module):
                ...
                if branch_number == branch_output_number:
                    ...
                    self.fuse_layers[-1].append(nn.Sequential(
                        ...
                        nn.Upsample(scale_factor=(
                            2.0 ** (branch_number - branch_output_number)), mode='nearest'),
                    ))
                elif branch_number < branch_output_number:
                    downsampling_fusion = []
    ...
    def forward(self, x):
        ...
        x = [branch(branch_input)
             for branch, branch_input in zip(self.branches, x)]
...
class HRNet(nn.Module):
    def __init__(self, c=48, num_blocks=[1, 4, 3], num_classes=1000):
        ...
        self.bn1 = nn.BatchNorm2d(
            64, eps=1e-05, affine=True, track_running_stats=True)
        ...
        self.transition1 = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(256, c, kernel_size=3, stride=1,
        ...
        self.bn_classifier = nn.Sequential(
            ...
            nn.BatchNorm2d(out_channels // 4, eps=1e-05,
                           affine=True, track_running_stats=True),
            nn.ReLU(inplace=True),
            nn.AdaptiveAvgPool2d(pool_feature_map),
            nn.Flatten(),
            nn.Linear(pool_feature_map * pool_feature_map *
