...
try:
    ...
    raise ImportError(
        f"[mamba_simple] 'einops' package is required but not found: {e}\n"
        "Please install it with: pip install einops"
    ) from e

# causal_conv1d is optional (CUDA acceleration)
try:
    from causal_conv1d import causal_conv1d_fn, causal_conv1d_update
except ImportError:
    causal_conv1d_fn, causal_conv1d_update = None, None

# selective_scan - try multiple import paths
try:
    from mamba_ssm.ops.selective_scan_interface import selective_scan_fn, mamba_inner_fn, bimamba_inner_fn, mamba_inner_fn_no_out_proj, SELECTIVE_SCAN_CUDA_AVAILABLE
except ImportError:
    try:
        from ..ops.selective_scan_interface import selective_scan_fn, mamba_inner_fn, bimamba_inner_fn, mamba_inner_fn_no_out_proj, SELECTIVE_SCAN_CUDA_AVAILABLE
    except ImportError as e:
        raise ImportError(
            f"[mamba_simple] Failed to import selective_scan_interface: {e}\n"
            "This is a critical dependency for Mamba."
        ) from e

# Check if selective_scan_fn is available
if selective_scan_fn is None:
    raise ImportError(
        "[mamba_simple] selective_scan_fn is None after import. "
        "This indicates a problem with the selective_scan_interface module."
    )

# Optional triton modules
try:
...
class Mamba(nn.Module):
    def __init__(
        ...
        self.D = nn.Parameter(torch.ones(
            self.d_inner, device=device))  # Keep in fp32
        ...
        if bimamba_type == "v1":
            ...
            self.D_b = nn.Parameter(torch.ones(
                self.d_inner, device=device))  # Keep in fp32
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        # Use fast path only if CUDA is available
        use_fast = self.use_fast_path and inference_params is None and SELECTIVE_SCAN_CUDA_AVAILABLE
        if use_fast:  # Doesn't support outputting the states
            ...
            # Handle bidirectional mamba v2 in slow path
    ...
    def step(self, hidden_states, conv_state, ssm_state):
        ...
        if causal_conv1d_update is None:
            ...
            conv_state[:, :, -1] = x
            x = torch.sum(conv_state * rearrange(self.conv1d.weight,
                          "d 1 w -> d w"), dim=-1)  # (B D)
