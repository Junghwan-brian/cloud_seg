...
def rms_norm_ref(
    ...
    out = ((x * rstd * weight) +
           bias if bias is not None else (x * rstd * weight)).to(dtype)
    if weight1 is None:
        return out if not prenorm else (out, x)
    else:
    ...
    if torch.version.hip:
        try:
            ...
            if 'instinct' in device_name.lower():
                ...
                warp_size = 32
            warnings.warn(
                f"{e}, warp size set to {warp_size} based on device name: {device_name}", UserWarning)
...
def _layer_norm_fwd_1pass_kernel(
    ...
    if HAS_DROPOUT:
        ...
        keep_mask = tl.rand(tl.load(SEEDS + row).to(tl.uint32),
    ...
    if HAS_X1:
        ...
        if HAS_DROPOUT:
            ...
            # 7 rounds is good enough, and reduces register pressure
            keep_mask = (
                tl.rand(tl.load(SEEDS + M + row).to(tl.uint32),
                        cols, n_rounds=7) > dropout_p
            )
            x1 = tl.where(keep_mask, x1 / (1.0 - dropout_p), 0.0)
            if STORE_DROPOUT_MASK:
                tl.store(DROPOUT_MASK + (M + row) * N +
                         cols, keep_mask, mask=cols < N)
        x += x1
    if HAS_RESIDUAL:
...
@triton.autotune(
    configs=pruned_configs_autotune,
    key=["N", "HAS_DRESIDUAL", "STORE_DRESIDUAL",
...
def _layer_norm_bwd_kernel(
    ...
    for row in range(row_start, row_end):
        ...
        if HAS_DROPOUT:
            keep_mask = tl.rand(
                tl.load(SEEDS + row).to(tl.uint32), cols, n_rounds=7) > dropout_p
...
class LayerNormFn(torch.autograd.Function):
    ...
    def forward(
        ...
        residual_out = residual_out.reshape(
            x_shape_og) if residual_out is not None else None
        dropout_mask = dropout_mask.reshape(
            x_shape_og) if dropout_mask is not None else None
        dropout_mask1 = dropout_mask1.reshape(
            x_shape_og) if dropout_mask1 is not None else None
