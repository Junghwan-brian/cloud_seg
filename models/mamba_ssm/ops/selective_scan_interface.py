...
try:
    ...
    raise ImportError(
        f"[selective_scan_interface] 'einops' package is required: {e}\n"
        "Install with: pip install einops"
    ) from e

# Optional CUDA accelerated causal_conv1d
try:
...
# Optional CUDA accelerated selective_scan
try:
    import selective_scan_cuda
    SELECTIVE_SCAN_CUDA_AVAILABLE = True
except ImportError:
    selective_scan_cuda = None
    SELECTIVE_SCAN_CUDA_AVAILABLE = False
    # This is expected when CUDA extensions are not compiled
    # Pure Python fallback (selective_scan_ref) will be used instead
...
def selective_scan_fn(u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False,
    ...
    """
    if SELECTIVE_SCAN_CUDA_AVAILABLE:
...
def mamba_inner_ref(
    ...
    x = causal_conv1d_fn(x, rearrange(
        conv1d_weight, "d 1 w -> d w"), conv1d_bias, activation="silu")
    # We're being very careful here about the layout, to avoid extra transposes.
    # We want delta to have d as the slowest moving dimension
    # and L as the fastest moving dimension, since those are what the ssm_scan kernel expects.
